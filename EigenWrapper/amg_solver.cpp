...
namespace amg {
	...
	amg_solver::amg_solver(size_t levels, size_t cycle, size_t iteration, size_t prev_smooth,
		size_t post_smooth, size_t fmg_inner_iteration, std::string smoother, std::string coarsener, std::string linear_solver, double tolerance/*const boost::property_tree::ptree &pt*/)
		/*: pt_(pt)*/ {
		nbr_levels_ = levels; // pt_.get<size_t>("#levels");
		nbr_inner_cycle_ = cycle; // pt_.get<size_t>("#cycle");
		nbr_outer_cycle_ = iteration; // pt_.get<size_t>("#iteration");
		nbr_prev_ = prev_smooth; // pt_.get<size_t>("#prev_smooth");
		nbr_post_ = post_smooth; // pt_.get<size_t>("#post_smooth");
		nbr_fmg_iter_ = fmg_inner_iteration; // pt_.get<size_t>("#FMG_inner_iteration");
		smooth_scheme_ = smoother; // pt_.get<string>("smoother");
		coarsen_scheme_ = coarsener; // pt_.get<string>("coarsener");
		linear_solver_ = linear_solver; // pt_.get<string>("linear_solver");
		tolerance_ = tolerance; // pt_.get<scalar>("tolerance");
